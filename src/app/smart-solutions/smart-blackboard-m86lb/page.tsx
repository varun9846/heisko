...
import { SmartBlackboardM86LB, SmartBlackboardM86LBResponse } from '@/lib/types'

/**
 * SmartBlackboardM86LB Series Smart Blackboard Page
 ...
 * 
 * SmartBlackboardM86LB Series Highlights:
 ...
 */
export default function SmartBlackboardM86LBPage() {
    const [products, setProducts] = useState<SmartBlackboardM86LB[]>([])
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState<string | null>(null)
    const [heroProduct, setHeroProduct] = useState<SmartBlackboardM86LB | null>(null)
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                setLoading(true)
                const response = await axios.get<SmartBlackboardM86LBResponse>('/api/smart-solutions/smart-blackboard-m86lb')

                if (response.data.success) {
                    console.log('✅ SmartBlackboardM86LB data fetched successfully:', response.data)
                    setProducts(response.data.data)
                    // Set the first product as hero product
                    if (response.data.data.length > 0) {
                        setHeroProduct(response.data.data[0])
                        console.log('🎯 SmartBlackboardM86LB Hero product set:', response.data.data[0])
                    }
                } else {
                    setError('Failed to fetch SmartBlackboardM86LB products')
                }
            } catch (err) {
                console.error('❌ Error fetching SmartBlackboardM86LB series data:', err)
                setError('Failed to load SmartBlackboardM86LB products. Please try again later.')
            } finally {
                setLoading(false)
            }
        }
    ...
    // SmartBlackboardM86LB-specific features highlighting smart blackboard capabilities
    const features = [
        {
            icon: (
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 text-lg font-medium">Loading SmartBlackboardM86LB Series...</p>
    ...
    return (
        <div className="min-h-screen">
            ...
            {/* Enhanced CTA Section with SmartBlackboardM86LB specific messaging */}
            <section className="py-24 bg-gradient-to-br from-red-600 to-red-800 relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <p className="text-lg md:text-xl text-red-100 mb-10 max-w-4xl mx-auto animate-fade-in-delay leading-relaxed">
                        Transform your classroom with our SmartBlackboardM86LB series.
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 justify-center animate-fade-in-delay-2">
                        <button className="bg-white text-red-600 hover:bg-red-50 px-8 py-4 text-lg font-semibold rounded-xl shadow-md hover:shadow-red-500/30 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-red-300">
                            Get SmartBlackboardM86LB Quote
                        </button>
                    ...
                    {/* Additional SmartBlackboardM86LB Features */}
